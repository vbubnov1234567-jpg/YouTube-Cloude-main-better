...
class YouTubeDecoder:
    ...
    def decode(self, video_file, output_dir='.'):
        ...
        # Поиск заголовка
        data_str = bytes_data[:1000].decode('utf-8', errors='ignore')
        ...
        if match:
            ...
            # ПРОВЕРКА: Если ты НЕ шифровал файл, программа должна это знать.
            # Допустим, мы проверяем переменную 'is_encrypted' (если она есть в коде)
            
            if self.key:
                # 1. Ключ есть — всё супер, открываем любое
                filename = real_name
                print(f"🔓 Файл восстановлен: {filename}")
            else:
                # 2. Ключа НЕТ. Проверяем, нужно ли оно тут вообще?
                # Если файл БЫЛ зашифрован (ты это знаешь по своим 4 сидам):
                if "сид" in real_name: # Простейшая проверка по имени для теста
                    filename = "ПОШЕЛ_ВОН_УЕБОК.txt"
                    print(f"⚠️ ОШИБКА: {filename}")
                    import sys
                    sys.exit()
                else:
                    # 3. Если это обычный файл (не сид) — открываем БЕЗ ШИФРА
                    filename = real_name
                    print(f"✅ Файл без шифра: {filename}")
                    # Здесь программа НЕ должна делать sys.exit(), чтобы файл сохранился!
